...
static int AP_Mount_set_roi_target(lua_State *L) {
    ...
    if (ud == nullptr) {
        return not_supported_error(L, 1, "mount");
    }
...
static int AP_Mount_set_mode(lua_State *L) {
    ...
    luaL_argcheck(L, ((raw_data_3 >= static_cast<int32_t>(MAV_MOUNT_MODE_RETRACT)) && (raw_data_3 <= static_cast<int32_t>(MAV_MOUNT_MODE_HOME_LOCATION))), 3, "argument out of range");
...
static int AC_AttitudeControl_get_rpy_srate(lua_State *L) {
    ...
    if (ud == nullptr) {
        return not_supported_error(L, 1, "AC_AttitudeControl");
    }
...
static int AP_Follow_get_target_heading_deg(lua_State *L) {
    ...
    if (ud == nullptr) {
        return not_supported_error(L, 1, "follow");
    }
...
static int OpticalFlow_index(lua_State *L) {
    const char * name = luaL_checkstring(L, 2);
    if (load_function(L,OpticalFlow_meta,ARRAY_SIZE(OpticalFlow_meta),name)) {
        return 1;
    }
    return 0;
}
...
static int Parameter_init_by_info(lua_State *L) {
    ...
    const uint32_t data_3 = static_cast<uint32_t>(raw_data_3);
    const lua_Integer raw_data_4 = luaL_checkinteger(L, 4);
    luaL_argcheck(L, ((raw_data_4 >= static_cast<int32_t>(AP_PARAM_INT8)) && (raw_data_4 <= static_cast<int32_t>(AP_PARAM_FLOAT))), 4, "argument out of range");
...
static int RC_Channel_set_override(lua_State *L) {
    ...
    const lua_Integer raw_data_2 = luaL_checkinteger(L, 2);
    luaL_argcheck(L, ((raw_data_2 >= MAX(0, 0)) && (raw_data_2 <= MIN(2200, UINT16_MAX))), 2, "argument out of range");
    const uint16_t data_2 = static_cast<uint16_t>(raw_data_2);
    ud->set_override(
            data_2,
            0);
...
#endif // HAL_WITH_ESC_TELEM == 1
#if AP_OPTICALFLOW_ENABLED
    {"optical_flow", OpticalFlow_index},
#endif // AP_OPTICALFLOW_ENABLED
    {"baro", AP_Baro_index},
